...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    mhd_list = glob.glob('/kaggle/input/luna16/subset[0-4]/subset[0-4]/*.mhd')

    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open('/kaggle/input/lunaproject/annotations_with_malignancy.csv', "r") as f:
...
class Ct:
    def __init__(self, series_uid):
        """
        mhd_path = glob.glob(
            'subset*/subset*/{series_uid}.mhd'.format(series_uid=series_uid)
        )[0]
        """
        p=os.path.join('/kaggle/input/luna16/subset[0-4]/subset[0-4]', series_uid + '.mhd')
        matches = glob.glob(p)

        if not matches:
            raise FileNotFoundError(f"No MHD file found for series_uid: {series_uid} in {p}")
